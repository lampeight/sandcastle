...
const DEFAULT_USERS = ["will", "darren", "nick", "ben"] as const;
...
const getUsers = (options?: CodexAuthRotationOptions): readonly string[] =>
  options?.users && options.users.length > 0 ? options.users : DEFAULT_USERS;
...
export const prepareCodexAuth = async (
  ...
  try {
    ...
    const selectedUser = nextUserInCycle(
      state?.lastAssignedUser ?? activeUser,
      users,
    );
    const { content } = await readSelectedAuthContent(
      dir,
      selectedUser,
      activeUser,
    );
